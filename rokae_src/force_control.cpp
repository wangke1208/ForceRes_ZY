...
int ForceControl::Fcinit() {
    ...
    std::vector<std::function<int()>> func_calls = {
        ...
        [this]() { return SetKpGain(m_init_robot_ptr->GetControlParams().m_gain_params.joint_gain_kp); },
        [this]() { return SetFricGain(m_init_robot_ptr->GetControlParams().m_gain_params.friction_cof_servo); },
...
int ForceControl::SetKpGain(const std::vector<double> kp_set) {
    ...
    m_fc_params_inner_ptr->m_function_params.SetParam("joint_servo_kp", kp_set);
    return SOLVE_NOERROR;
}

int ForceControl::SetFricGain(const std::vector<double> fric_set) {
    if (m_jnt_num != fric_set.size()) {
        return SIZE_ERROR;
    }
    ...
    m_fc_params_inner_ptr->m_function_params.SetParam("joint_servo_friction", fric_set);
