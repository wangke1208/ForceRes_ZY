...
/**
 * @class ForceControl
 * @brief 力控模块的单例类，负责力控相关的计算和参数管理
 * 
 * 该类采用单例模式，确保整个程序中只有一个力控模块实例。
 */
class ForceControl {
private:
 // 常量定义
 ...
 // 单例相关
 static ForceControl* force_control_instance_ptr;
...
public:
  ...
  * @brief 重置力控内部状态
  *
  * 此函数用于根据伺服模式重置力控状态,只允许位置模式调用，且用于当Drag_Config或Fc_Update返回错误时调用。调用后需要，重新设置各种增益以及负载参数，零点和传感器相关参数不用重新设置
  *
  * @param[in] servo_mode 输入：伺服模式
  * @return 输出：成功返回 SOLVE_NOERROR，失败返回相应的错误码
  */
 int ResetFcStatus(const std::vector<int8_t>& servo_mode);
