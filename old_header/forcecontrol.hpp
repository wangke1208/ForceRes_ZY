...
class ForceControl {
   public:
     ...
     */
    int DragConfig(const int8_t param_0x6061[6], const int16_t param_0x2401[6], const int16_t param_0x2402[6], const int32_t param_0x6064[6],
     ...
     */
    int FcUpdate(const int32_t param_0x6064[6], const int32_t param_0x606C[6], const int16_t param_0x2406[6], const int8_t param_0x6061[6],
     ...
     */
    int DataTransform(const int32_t param_0x6064[6], const int32_t param_0x606C[6], const int16_t param_0x2406[6], const int8_t param_0x6061[6]);
     ...
     */
    int UpdateFcStatus(const int32_t param_0x6064[6], const int32_t param_0x606C[6], const int16_t param_0x2406[6], const int8_t param_0x6061[6]);
     ...
     * @return 无
     */    
    void ServoDataVectorToArray(int16_t param_0x6071[6], int16_t param_0x60B2[6], int16_t param_0x2201[6], int16_t param_0x2202[6], int16_t param_0x2203[6],
                                int16_t param_0x2204[6], int16_t param_0x2205[6], int16_t param_0x2206[6]);
    ...
    */     
     ...
     */
    bool IsInForceControlArea(const JntArray& q_in, const std::vector<double>& joint_limit_upper, const std::vector<double>& joint_limit_lower);
     ...
     *                
     */
    int CalibrateTrqSensor(const int32_t param_0x6064[6], const LoadInertia& load_params_in, const int16_t analog_array_ch1[6][200], const int16_t analog_array_ch2[6][200], double sensor_bias[6]);
     ...
     *
     */
    int CalibrateTrqSensorAxis(const int32_t param_0x6064[6], const LoadInertia& load_params_in, const int16_t analog_array_ch1[200],
   ...
   private:
    ...
    KDL::Chain m_chain;
    FcStatusInfo m_fc_status;
    FcStatusTracker* m_fc_status_tracker;
    MechanicalParams m_mechanical_params;
    ControlParams m_control_params;
    ModelParams m_model_params;
    ServoToFc* m_servo_to_fc;
    FcToServo m_fc_to_servo;
    DynamicSolver* m_fc_dynamic_solver;
    Control::ForcePlanner* m_force_planner_ptr;

    // fcplanner计算的力矩
    KDL::JntArray m_ref_trq_desire;
    KDL::JntArray m_ref_trq_overlay;
    KDL::JntArray m_ref_trq_impedance;
    KDL::JntArray m_ref_trq_virtual_wall;
    KDL::JntArray m_ref_trq_joint_limit;
    KDL::JntArray m_ref_trq_dyn;
    KDL::JntArray m_ref_trq;
    ...
    //编码器
    std::vector<int32_t> m_encoder_offset;
    std::vector<int> m_encoder_resolution;
    std::vector<double> m_decel_ratio_high;
    std::vector<double> m_decel_ratio_low;
    std::vector<double> m_rated_torque;
    //传感器
    std::vector<double> m_analog2trq_high;
    std::vector<double> m_analog2trq_low;
    std::vector<double> m_analog_bias;
    std::vector<double> m_sensor_amplify;
    std::vector<double> m_encoder_to_jnt_scale;
    std::vector<double> m_decel_ratio;
    std::vector<int16_t> m_analog_ch1;
    std::vector<int16_t> m_analog_ch2;
    std::vector<int16_t> m_analog_average;
    std::vector<double> m_analog2trq;
    std::vector<double> m_sensor_trq;

    //模型参数
    std::vector<double> m_joint_range_min;
    std::vector<double> m_joint_range_max;
    std::vector<double> m_joint_range_min_new;
    std::vector<double> m_joint_range_max_new;

    //控制参数
    std::vector<double> m_joint_gain_kp;
    std::vector<double> m_joint_damp_zeta;
    std::vector<double> m_friction_cof_servo;
    std::vector<double> m_joint_inertia;
    std::vector<double> m_ref_trq_vec;
    std::vector<double> m_trq_feedforward;

    //一些保护参数
    std::vector<double> m_trq_error;
    std::vector<double> m_trq_error_max;
    std::vector<double> m_trq_comp_coef;
